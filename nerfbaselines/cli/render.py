...
from nerfbaselines.types import Method, get_args
from nerfbaselines import backends
from nerfbaselines import registry
from nerfbaselines.evaluation import render_all_images, render_frames, trajectory_get_embeddings, trajectory_get_cameras, OutputType
...
def render_trajectory_command(checkpoint: Union[str, Path], 
    ...
    is_video = (output.endswith(".mp4") or 
                output.endswith(".avi") or 
                output.endswith(".gif") or 
                output.endswith(".webp") or
                output.endswith(".mov"))
    if is_video and len(output_names) > 1 and "{output}" not in output:
        logging.critical("Output video can only contain a single output name unless the path contains '{output}'")
        sys.exit(1)
