...
class GaussianSplattingWild(Method):
        ...
        with torch.no_grad():
            for i, pose in enumerate(poses):
                ...
                if emb is not None:
                    ...
                    use_cache = True
                rendering = render(viewpoint, self.gaussians, self.pipe, self.background, use_cache=use_cache, store_cache=True)
    ...
    def optimize_embeddings(
        ...
        for i in range(len(dataset["cameras"].poses)):
            ...
            if embeddings is not None and embeddings[i] is not None:
                ...
                for _ in self.render(dataset["cameras"][i:i+1], embeddings=embeddings):
                    pass
    ...
    def get_train_embedding(self, index: int) -> Optional[np.ndarray]:
        ...
        """
        del index
        # TODO: Implement getting embeddings in training mode
        raise NotImplementedError("Method supports optimizing embeddings, but it does not store training embeddings")
